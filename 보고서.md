제공해주신 파일들(`bldcm23 (5).pdf`, `아두이노 BLDC모터 구동1.pdf`, `BLDCM_drivercontrol.pdf`, `스텝-초음파모터.pdf`, `회전기 영구자석.pdf`)을 바탕으로 **BLDCM 구동 장치 제작 보고서**를 작성해 드립니다.

Github에 저장할 수 있도록 Markdown 포맷으로 작성되었습니다.
...
BLDC(Brushless DC) 모터는 기계적인 브러시와 정류자가 없는 직류 전동기입니다. [cite_start]기존 DC 모터가 브러시를 통해 기계적으로 전류의 방향을 바꾸는 것과 달리, BLDC 모터는 반도체 소자(트랜지스터 등)를 이용하여 전기적으로 전류의 방향(전류, Commutation)을 제어합니다[cite: 3497, 3498].

  * [cite_start]**회전자(Rotor):** 영구자석으로 구성되어 있으며 회전하는 부분입니다[cite: 3497].
  * **고정자(Stator):** 권선(Coil)이 감겨 있어 자계를 형성하는 부분입니다.
  * [cite_start]**위치 센서:** 회전자의 자극 위치를 검출하기 위해 주로 홀 센서(Hall Sensor)를 사용합니다[cite: 3499].

### 1.2 6-Step 구동 (120도 통전 방식)

BLDC 모터의 가장 일반적인 구동 방식은 3상 권선에 120도 위상차를 두고 전류를 흘려주는 6-Step 구동 방식입니다.

1.  [cite_start]**홀 센서 신호:** 3개의 홀 센서($H_a, H_b, H_c$)가 회전자의 N극과 S극을 감지하여 디지털 신호(High/Low)를 출력합니다[cite: 4259].
2.  **스위칭 시퀀스:** 홀 센서의 신호 조합(State)에 따라 3상 인버터의 6개 스위치(TR 또는 FET) 중 2개(상단 1개, 하단 1개)를 켭니다. [cite_start]이를 통해 고정자에 회전 자계가 형성되어 회전자를 끌어당기거나 밀어내며 회전력을 발생시킵니다[cite: 3697, 3763].
...
아두이노를 컨트롤러로 사용하여 3상 인버터 회로를 구성합니다. 제공된 자료에 따른 회로 구성은 다음과 같습니다.
  ...
  * [cite_start]**MCU:** Arduino Uno[cite: 4329].
  * **3상 인버터 (Bridge Circuit):**
      * [cite_start]**High-Side (상단 스위치):** TIP115 (PNP Darlington Transistor) 사용[cite: 4363, 5260]. PNP 타입이므로 Base가 Low일 때 켜집니다.
      * [cite_start]**Low-Side (하단 스위치):** IRF530 (N-Channel MOSFET) 사용[cite: 4362, 5222]. Gate가 High일 때 켜집니다.
      * [cite_start]**Pre-Driver:** 아두이노의 5V 신호로 TIP115(12V\~24V 라인)를 제어하기 위해 2N3904 (NPN 트랜지스터)를 전단에 배치하여 레벨 시프팅 및 스위칭을 담당합니다[cite: 4351].
  * [cite_start]**홀 센서 입력:** 모터 내부의 홀 센서($H_a, H_b, H_c$) 출력을 아두이노의 디지털 핀(2, 4, 7번 등)에 연결하며, 풀업(Pull-up) 저항을 사용하여 신호를 안정화합니다[cite: 4247].

### 2.2 구동 알고리즘 (Commutation Logic)

아두이노는 홀 센서의 값을 읽어 현재 회전자의 위치(Sector)를 파악하고, 그에 맞는 상(Phase)에 전압을 인가합니다.

  * **상태 변환:** 3개의 홀 센서 값을 이진수로 조합하여 하나의 상태 값(State)으로 변환합니다.
    [cite_start]$$State = 4 \times H_a + 2 \times H_b + 1 \times H_c$$[cite: 4541, 5429].
  * **스위칭 테이블:** 계산된 State에 따라 미리 정의된 스위칭 테이블대로 핀 출력을 제어합니다. [cite_start]예를 들어 `State == 5` ($H_a=1, H_b=0, H_c=1$)인 경우, $S_A^+$ (상단)와 $S_B^-$ (하단) 스위치를 켭니다 [cite: 5432-5446].
...
모터의 속도를 일정하게 유지하거나 목표 속도에 도달하게 하기 위해 PI(비례-적분) 제어를 사용합니다.
  ...
  * **속도 측정 (T-method):** 홀 센서 신호의 Rising Edge 사이의 시간(주기, Period)을 측정하여 RPM을 계산합니다. [cite_start]저속에서도 정밀한 측정이 가능합니다[cite: 4572, 6084].
    $$ RPM = \frac{120 \times Frequency}{\text{Pole Number}} $$
  * **오차(Error) 계산:** $$Error = \text{Target RPM} - \text{Current RPM}$$
  * **PI 연산:**
      * **P(비례) 제어:** 현재 오차에 비례하여 제어량을 조절합니다. [cite_start]$$P = K_p \times Error$$[cite: 6862].
      * **I(적분) 제어:** 오차의 누적값을 통해 잔류 편차(Steady-state error)를 제거합니다. [cite_start]$$I = I + (Error \times K_i \times dt)$$[cite: 4862, 6090].
  * [cite_start]**출력(PWM Duty):** 최종적으로 계산된 PI 값을 현재의 PWM Duty Cycle에 더하여 모터 전압을 조절합니다[cite: 6436].
...
1.  **초기화 (`setup`)**: `attachInterrupt`를 사용하여 홀 센서 A의 신호 변화(Rising Edge)를 감지합니다. [cite_start]또한, 정밀한 제어 루프를 위해 아두이노의 Timer1 레지스터를 직접 조작하여 200us마다 인터럽트가 발생하도록 설정합니다[cite: 6311].
2.  **속도 측정 (`function`)**: 홀 센서 인터럽트가 발생할 때마다 현재 시간(`micros()`)을 측정하고 이전 시간과의 차이(주기)를 구합니다. [cite_start]이 주기의 역수를 취해 주파수를 구하고, 이를 RPM으로 변환합니다[cite: 5816].
3.  **PI 제어 및 스위칭 (`ISR`)**:
      * **상태 판별:** 3개의 홀 센서 값을 읽어 1\~6 사이의 State 값을 만듭니다.
      * [cite_start]**PI 연산:** 가변저항으로 설정한 목표 RPM과 현재 RPM의 차이(Error)를 구하고, P게인과 I게인을 적용하여 모터에 인가할 전압 비율(Duty Cycle)을 계산합니다[cite: 6435].
      * **전자적 정류(Commutation):** 계산된 State 값에 따라 `switch-case` 문을 사용하여 3상 인버터의 TR을 켭니다. [cite_start]속도 조절을 위해 High-Side 트랜지스터 핀에는 `analogWrite`로 PWM 신호(Duty)를 내보냅니다[cite: 6536].
